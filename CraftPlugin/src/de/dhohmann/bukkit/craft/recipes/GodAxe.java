...
/**
 * Recipe for a godlike axe that will destroy a block by hitting it once.
 * @author dhohmann
 *
 */
public class GodAxe extends CraftRecipe implements Toggleable, GodLike {
	...
	if (!enabled) {
	    ...
	    meta.setDisplayName(language.getString("recipes.godaxe.name", "GodAxe"));
	    List<String> lore = new ArrayList<>();
	    lore.add(language.getString("recipe.godaxe.description", "Hit a block and break it instantly."));
	    meta.setLore(lore);
	...
	if (enabled) {
	    ...
	    while (iter.hasNext()) {
		if (iter.next() instanceof GodAxe) {
		    iter.remove();
		}
	    }
	}
