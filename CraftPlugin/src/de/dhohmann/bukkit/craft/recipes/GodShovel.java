...
/**
 * Recipe for a godlike shovel that will destroy a block by hitting it once.
 * @author dhohmann
 *
 */
public class GodShovel extends CraftRecipe implements Toggleable, GodLike {
    ...
    /**
     * Creates a godlike shovel recipe
     * @param lang Language model used for the description
     * @param mode Mode of the god like behaviour
     */
    public GodShovel(FileConfiguration lang, int mode) {
	super(new ItemStack(Material.DIAMOND_SPADE, 1));
	language = lang;
	enabled = false;
	this.mode = mode;
    }
	...
	if (!enabled) {
	    ...
	    meta.setDisplayName(language.getString("recipes.godshovel.name", "GodShovel"));
	    List<String> lore = new ArrayList<>();
	    lore.add(language.getString("recipe.godshovel.description", "Hit a block and break it instantly."));
	    meta.setLore(lore);
	    ...
	    recipe.setIngredient('B', Material.DIAMOND_SPADE);
	...
	if (enabled) {
	    ...
	    while (iter.hasNext()) {
		if (iter.next() instanceof GodShovel) {
		    iter.remove();
		}
	    }
	}
