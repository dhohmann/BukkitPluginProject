...
/**
 * Recipe for a unbreakable godlike shears.
 * @author dhohmann
 *
 */
public class GodShears extends CraftRecipe implements Toggleable, GodLike {
    ...
    /**
     * Creates a godlike shears recipe
     * @param lang Language model used for the description
     * @param mode Mode of the god like behaviour
     */
    public GodShears(FileConfiguration lang, int mode) {
	super(new ItemStack(Material.SHEARS, 1));
	language = lang;
	enabled = false;
	this.mode = mode;
    }
	...
	if (!enabled) {
	    ...
	    meta.setDisplayName(language.getString("recipes.godshears.name", "GodShears"));
	    List<String> lore = new ArrayList<>();
	    lore.add(language.getString("recipe.godshears.description", "Hit a block and break it instantly."));
	    meta.setLore(lore);
	    ...
	    recipe.setIngredient('B', Material.DIAMOND_AXE);
	...
	if (enabled) {
	    ...
	    while (iter.hasNext()) {
		if (iter.next() instanceof GodShears) {
		    iter.remove();
		}
	    }
	}
